...
A dead simple & powerful way of adding rich & adaptive animations to your app which is already using Turbo™ Rails™.
...
```javascript
TurboRailsAnimate.init({ animation: "fadeinright" });
```
...
```javascript
TurboRailsAnimate.init({ animation: "fadeinright", duration: "1s", delay: 1000 });
```
...
<a
  href=""
  data-turbo-rails-animate-animation="fadeout"
  data-turbo-rails-animate-duration="0.3s"
  data-turbo-rails-animate-delay="250"
  >I am a link!</a
>
```
...
- `data-turbo-rails-animate-animation` Animation to be applied when disappearing after a hyperlink got clicked. Accepts a string. Set it to `'false'` to disable Turbolinks Animate on this specific link.

- `data-turbo-rails-animate-appear` Animation to be applied when appearing on the next view after a hyperlink got clicked. Accepts a string.

- `data-turbo-rails-animate-duration` CSS value for `animation-duration`. Accepts a string.

- `data-turbo-rails-animate-delay` Milliseconds after which animation starts. Accepts an integer or `false`.
...
```javascript
TurboRailsAnimate.init({ animation: { mobile: "fadeinup", tablet: "fadeindown", desktop: "fadein" } });
```
...
```html
<body data-turbo-rails-animate-animation="fadeinup"></body>
```
...
```html
<body data-turbo-rails-animate-animation="fadein">
  <h1 data-turbo-rails-animate-persist="true">My app</h1>
...
```html
<body data-turbo-rails-animate-animation="fadein">
  <nav data-turbo-rails-animate-persist-itself="nav" style="background: black;">
    <h1 style="color: white;">View specific title</h1>
  </nav>
  <a href="/do" data-turbo-rails-animate-type="nav">Persist navigation!</a>
...
```html
<header data-turbo-rails-animate-persist-itself="true" data-turbo-rails-animate-transition="background-color,opacity">
  <!-- ... -->
</header>
```
...
- `turbo:animation-start` fires when an animation starts. The main Turbolinks Animate element can be accessed with `event.data.element`. Access the animation with `event.data.animation`. Access whether content appears or disappears with `event.data.disappearing`.

- `turbo:animation-end` fires when an animation ends. The main Turbolinks Animate element can be accessed with `event.data.element`. Access whether content appeared or disappeared with `event.data.disappearing`.
...
We use [GitHub projects](https://github.com/mes-amis/turbo-rails-animate/projects/1) to coordinate the work on this project.

To propose your ideas, initiate the discussion by adding a [new issue](https://github.com/mes-amis/turbo-rails-animate/issues/new).
